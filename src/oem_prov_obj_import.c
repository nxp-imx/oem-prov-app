			 ...
			 size_t length)
{
	...
	OEM_PROV_DBG_PRINTF(VERBOSE, "Importing blob (id: 0x%08x)\n", blob_id);
	...
	if (blob_id != SRKH_KEY_ID) {
		...
		if ((status != PSA_SUCCESS) &&
			...
			OEM_PROV_PRINTF("Blob (id: 0x%08x) import: FAILED\n",
					blob_id);
	...
	if ((status != PSA_SUCCESS) && (blob_id == SRKH_KEY_ID)) {
		OEM_PROV_DBG_PRINTF(INFO, "SRKH already imported\n");
	...
	OEM_PROV_PRINTF("Blob (id: 0x%08x) import: SUCCESS\n", ret_blob_id);
	return OEM_PROV_STATUS_OK;
}
